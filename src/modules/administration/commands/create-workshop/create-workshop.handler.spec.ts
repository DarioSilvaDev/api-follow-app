...
import { CreateWorkshopHandler } from './create-workshop.handler';
...
describe('CreateWorkshopHandler (admin) — D-106 onboarding administrado', () => {
  let handler: CreateWorkshopHandler;
  let prismaMock: {
    workshop: { findFirst: jest.Mock; findUnique: jest.Mock };
  ...
  beforeEach(() => {
    prismaMock = {
      workshop: { findFirst: jest.fn(), findUnique: jest.fn() },
      permission: { findMany: jest.fn() },
      $transaction: jest.fn(),
    };
    eventEmitterMock = { emit: jest.fn() };
    handler = new CreateWorkshopHandler(
      prismaMock as any,
  ...
  const permissionStub = [
    { id: 'p-member.invite', code: 'member.invite' },
    { id: 'p-workshop.update', code: 'workshop.update' },
    { id: 'p-appointment.create', code: 'appointment.create' },
    { id: 'p-workorder.create', code: 'workorder.create' },
    { id: 'p-service-record.create', code: 'service-record.create' },
  ...
  const setupHappyPath = (overrides: Record<string, unknown> = {}) => {
    ...
    const tx = txMock();
    tx.workshop.create.mockResolvedValue({
      id: 'workshop-1',
      name: 'Taller Nuevo',
      taxId: '30123456789',
    ...
    tx.workshopRole.findUnique.mockResolvedValue({ id: 'role-owner' });
  ...
  it('crea el taller en pending_claim con roles default e invitación (sin exponer token)', async () => {
    ...
    // Transaccional: roles creados con la matriz del seed + invitación owner.
    const createData = tx.workshop.create.mock.calls[0][0].data;
  ...
  it('emite workshop.member.invited con token (para el mail del wizard)', async () => {
    setupHappyPath();

    await handler.execute(
      new CreateWorkshopCommand(
        { name: 'Taller Nuevo', ownerEmail: 'dueno@example.com' },
        'admin-1',
      ),
    );
    ...
    expect(eventName).toBe('workshop.member.invited');
  ...
  it('rechaza con 409 CONFLICT si el nombre ya existe (case-insensitive)', async () => {
    prismaMock.workshop.findFirst.mockResolvedValue({ id: 'existing' });

    await expect(
      handler.execute(
        new CreateWorkshopCommand(
          { name: 'taller nuevo', ownerEmail: 'a@example.com' },
          'admin-1',
        ),
      ),
  ...
  it('rechaza con 409 CONFLICT si el CUIT ya existe (pre-check normalizado)', async () => {
    ...
    prismaMock.workshop.findUnique.mockResolvedValue({ id: 'existing' });
