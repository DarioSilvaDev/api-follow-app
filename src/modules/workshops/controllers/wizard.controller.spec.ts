...
// 'THROTTLER:LIMIT<name>' / 'THROTTLER:TTL<name>' (ver dist/throttler.constants).
// El throttler sin nombre se llama 'default'.
const THROTTLER_LIMIT_DEFAULT = 'THROTTLER:LIMITdefault';
const THROTTLER_TTL_DEFAULT = 'THROTTLER:TTLdefault';

/**
 * WizardController (workshops) — D-106 wizard público de onboarding.
 *
 ...
 */
describe('WizardController (workshops) — D-106 wizard público (routes / guards / throttle SC-3)', () => {
  const proto = WizardController.prototype as Record<string, unknown>;
  ...
  it('no aplica guards a nivel de clase (wizard público por token de invitación)', () => {
    expect(Reflect.getMetadata(PATH_METADATA, WizardController)).toBe(
      'workshops/wizard',
    );
    const classGuards = Reflect.getMetadata(GUARDS_METADATA, WizardController);
