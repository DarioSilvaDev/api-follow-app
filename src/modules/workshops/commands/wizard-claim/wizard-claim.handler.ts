...
/**
 * D-106: claim del wizard de onboarding de taller.
 *
 * Flujo (transaccional):
 * 1. Valida token/invitación (404 inválida, 400 expirada, 409 usada/cancelada).
 * 2. Valida que el taller siga `pending_claim`.
 ...
 * 5. Acepta la invitación, marca el taller active + claimed_at y crea el
 *    membership owner (rol owner del seed, sin mecanismo paralelo).
 *
 * Eventos posteriores al commit: workshop.member.joined, workshop.claimed
...
export class WizardClaimHandler {
  ...
  async execute(command: WizardClaimCommand) {
    ...
    if (!existingUser) {
      ...
      if (!command.sessionUserId) {
        throw new AuthRequiredException(
          'Debes iniciar sesión para vincular tu cuenta al taller',
        );
      }
    ...
    if (dto.workshop) {
      if (dto.workshop.email) workshopPatch.email = dto.workshop.email;
      if (dto.workshop.phone !== undefined) {
        workshopPatch.phone = dto.workshop.phone;
      }
      if (dto.workshop.website !== undefined) {
        workshopPatch.website = dto.workshop.website;
      }
      if (dto.workshop.description !== undefined) {
        workshopPatch.description = dto.workshop.description;
      }
    }
    ...
    try {
      const result = await this.prisma.$transaction(async (tx) => {
        ...
        const invitationUpdate = await tx.workshopInvitation.updateMany({
          where: { id: invitation.id, status: 'pending' },
          data: { status: 'accepted', acceptedAt: now },
        });
        if (invitationUpdate.count === 0) throw new InvitationUsedException();

        const workshopUpdate = await tx.workshop.updateMany({
          where: { id: invitation.workshopId, status: 'pending_claim' },
          data: workshopPatch,
        });
        if (workshopUpdate.count === 0) throw new InvitationUsedException();
        ...
        if (!ownerRole) {
          throw new Error(
            `Owner role missing for workshop ${invitation.workshopId}`,
          );
        }

        const member = await tx.workshopMember.create({
          data: {
            workshopId: invitation.workshopId,
            userId: userId!,
            roleId: ownerRole.id,
        ...
        const workshop = await tx.workshop.findUniqueOrThrow({
          where: { id: invitation.workshopId },
        });

        return { user: user!, member, workshop };
      });
      ...
      this.eventEmitter.emit(
        ...
        new MemberJoinedEvent(invitation.workshopId, userId!),
      );

      this.eventEmitter.emit(
        'workshop.claimed',
        new WorkshopClaimedEvent(invitation.workshopId, userId!, email),
      );
      ...
      return {
        ...
        workshop: {
          ...
          phone: result.workshop.phone,
          website: result.workshop.website,
          description: result.workshop.description,
        },
        member: {
          id: result.member.id,
