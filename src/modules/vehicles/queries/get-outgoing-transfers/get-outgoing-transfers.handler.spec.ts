import { GetOutgoingTransfersHandler } from './get-outgoing-transfers.handler';

describe('GetOutgoingTransfersHandler — D-078 symmetric contract (no PII)', () => {
  let handler: GetOutgoingTransfersHandler;
  let prismaMock: {
  ...
  it('filters by fromUserId and orders by createdAt desc', async () => {
    prismaMock.vehicleTransfer.findMany.mockResolvedValue([]);

    await handler.execute('u-yo');

    const arg = prismaMock.vehicleTransfer.findMany.mock.calls[0][0];
    expect(arg.where).toEqual({ fromUserId: 'u-yo' });
